...
<div class="row">
  ...
  <div class="col-sm-1 col-1-harf col-xs-6 padding-5">
	...
	<div class="col-sm-3 col-xs-12">
		...
		<label>
			<input type="checkbox" class="ace" name="hideItem" id="hideItem" value="Y" checked />
	...
	<div class='modal-dialog' id='modal' style="width:500px;">
        ...
        <?php if(!empty($groupList)) : ?>
          <?php foreach($groupList as $rs) : ?>
            <div class="col-sm-12">
              <label>
                <input type="checkbox" class="group-chk" id="group-<?php echo $rs->code; ?>" name="itemGroup[]" value="<?php echo $rs->code; ?>" style="margin-right:10px;" />
                <?php echo $rs->code; ?> | <?php echo $rs->name; ?>
              </label>
            </div>
          <?php endforeach; ?>
...
<script id="template" type="text/x-handlebars-template">
  <table class="table table-bordered" style="width:100%; min-width:1200px; table-layout:fixed;">
    <tr class="font-size-12">
      <th class="th middle text-center" style="width:50px;">#</th>
			<th class="th middle" style="width:200px;">Item Number</th>
      <th class="th middle" style="width:200px;">Item Description</th>
      <th class="th middle" style="width:50px;">UoM</th>
      <th class="th middle" style="width:100px;">Whs Total</th>
			{{#each header}}
				<th class="th middle" style="width:100px;">{{whsCode}}</th>
			{{/each}}
    </tr>
{{#each data}}
    <tr class="font-size-12">
      ...
      <td class="middle text-right" >{{ whTotal }}</td>
      {{#each whQty}}
...
<script src="<?php echo base_url(); ?>scripts/report/stock_balance_by_warehouse.js?v=<?php echo date('YmdH'); ?>"></script>
